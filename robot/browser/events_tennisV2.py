...
from settings import TENNIS_URL
...
EXCLUSION_LIST = ["ver todo", "total look"]
INCLUSION_LIST = ["ropa-mujer", "ropa-hombre", "ninas", "ninos", "unisex", "mascotas"]
        ...
        try:
            ...
            for category in INCLUSION_LIST:
                ...
                while True:
                    ...
                    url_category = 'https://www.tennis.com.co/_v/segment/graphql/v1?workspace=master&maxAge=short&appsEtag=remove&domain=store&locale=es-CO&operationName=Products&variables={}&extensions={"persistedQuery":{"version":1,"sha256Hash":"9b475e0aef97f309715db0071b1c7430f237580f86ce06a53e946ca823ec24bd","sender":"vtex.store-resources@0.x","provider":"vtex.search-graphql@0.x"},"variables":"%s"}' % query.decode()
            ...
            logging.error(
                (f"Tennis An error has ocurred during scan category: {category}"))
            logging.error(
                (f"details: {e}"))
...
# ------------------------------------------------------------------------------------------------ #
    ...
    def get_product_details(self, count_product, item):
        try:
                ...
                item_characteristics = item.get("description")

                materials = ""
                for comp in composition_items:
                    if "Material" in comp.get('name'):
                        materials += ",".join(comp.get('values'))

                materials = materials.replace("\n", "").replace("\r", "")
                patron = r'\b(?:Color:).*?<br />'
                # patron_2 = r'\b(?:Pa[ií]s de Origen).*?</li>'
                
                color_name = re.findall(patron, item_characteristics, re.IGNORECASE)

                patron_3 = r'\b(?::).*?<'
                color_name = re.findall(patron_3, color_name[0], re.IGNORECASE)[0].strip(":<").strip() if len(color_name) else None
                ...
                print(f"Tennis {category} {subcategory} {subcategory_2}  {subcategory_3} item {count_product}\t{round(time.time()-start, 3)} s")
            
                time.sleep(0.2)   
        except Exception as e:
            print(e)
            logging.error(f"Tennis...Error get product details {category} {subcategory} {subcategory_2}... {type(e)} Message: {e}")
