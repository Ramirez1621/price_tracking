...
class aeSession():
    ...
    def start_scrapping(self, semaphore):
        with semaphore:
            ...
            logging.info(f"Excution American Eagle Time: {end-start} s")

            backup_dataset = join(BASE_DIR,"Backup")
            df = pd.DataFrame.from_records(self.records)
            df.to_csv(join(backup_dataset, f"dataset_AE.csv"), index=False)
        # return self.records

    def open_page(self, driver: Firefox):
        try:
            driver.get(url=AE_URL)
    ...
    def select_categories(self, driver: Firefox):
        try:
            ...
            categories = WebDriverWait(driver, 5).until(EC.presence_of_all_elements_located(
                        (By.XPATH, "//div[contains(@class,'americaneagle-americaneagle-5-x-menud_container_AE')]//li//div/div/div/a")))
            
            categories_outitem = WebDriverWait(driver, 5).until(EC.presence_of_all_elements_located(
                        (By.XPATH, "//div[contains(@class,'americaneagle-americaneagle-5-x-menu_item_out')]/a")))
            categories.extend(categories_outitem)
            categories = [c.get_attribute("href") for c in categories if "aerie" not in c.get_attribute("href").lower() and "ver todo" not in c.get_attribute("innerText").lower() and "rebajas" not in c.get_attribute("href").lower() and "new-arrivals" not in c.get_attribute("href").lower() and "offline" not in c.get_attribute("href").lower() and "ultimas-tallas" not in c.get_attribute("href").lower()]
    ...
    def get_product_details(self, driver: Firefox, items: list[dict]):
        for i, item in enumerate(items):
            try:
                    ...
                    self.records.append(
                            {
                                ...
                                "shipment cost": stock,
                                "sales flag": saving,
                                "store id": f"9999_americaneagle_col",
                                "store name": "ONLINE",
                ...
                # print(f"AE item {i+1}/{len(items)}\t{round(time.time()-start, 3)} s")
                print(f"AE {category} {subcategory} {subcategory_2}  {subcategory_3} item {i+1}/{len(items)}\t{round(time.time()-start, 3)} s")
                
            except Exception as e:
                print(e)
                logging.error(f"AE...Error get product details {category} {subcategory} {subcategory_2}: {e}")
