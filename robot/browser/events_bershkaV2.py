...
# ------------------------------------------------------------------------------------------------ #
    def select_categories(self):
        try: 
            ...
            url_category = f"https://static.bershka.net/4/static/itxwebstandard/config/grid_carousel.json?t={consult_date}"
...
# ------------------------------------------------------------------------------------------------ #
    ...
    def get_product_details(self, count_product, item):
        try:
                ...
                # category_dict = {"WOMEN": "Mujer", "MEN":"Hombre", "KID": "Niños"}
                href = item.get("productUrl")
                href = f"https://www.bershka.com/co/{href}"
                ...
                subcategory = item.get("familyNameEN")
                subcategory_2 = item.get("subFamilyNameEN")
                

                categories = []
                for c in item.get("relatedCategories"):
                    if c["name"] not in categories and not "ver todo" in c["name"].lower() and not "lo más vendido" in c["name"].lower() and not "hasta" in c["name"].lower() and not "probador" in c["name"].lower() and not "special prices" in c["name"].lower() and not "new" in c["name"].lower():
                ...
                if subcategory is None and len(categories) > 1:
                ...
                description = item_details.get("detail").get("description")
                ...
                for detail in item_details.get("detail").get("colors"):
                    ...
                    image_url = f"https://static.bershka.net/4/photos2{image}_1_1_0.jpg?imwidth=:width"
